...
import { Instruction, ModeInstruction } from "./Instruction";
...
export const MapDialog: Component = () => {
    return (
        <Dialog isOpen={dialogSys.isMapDialogOpen}
                ...
                title="">
            <div class={DialogTitleStyle}>{mainSys.curMap.name}</div>
            <MapInfoSection />
        </Dialog>
    )
}
...
export const SettingDialog: Component = () => {
  return (
    <Dialog
      ...
      title="키 설정"
    >
      <Show when={mainSys.curUser.id === -1}>
        <p class={css({alignSelf: 'center', color: Color.grayDark})}>※ 키 설정은 로그인 후 가능합니다.</p>
      </Show>
      <KeySetGrid />
    </Dialog>
  );
      ...
      <div class={OverlayStyle}>
        <div class={`${DialogStyle} ${css({gap: Size.space.xl})}`}>
          ...
          <button class={buttonStyle}
                  onClick={() => {
                    requestSys.increaseRating(mainSys.curMap.id);
