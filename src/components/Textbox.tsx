...
import { accountSys, accountType } from "../systems/Account";
...
export const AccountTextbox: Component<{
  ...
  return (
    <input
      class={TextboxStyle(Size.ui.LoginW, accountSys.emailError())} // Add ErrorGlowStyle conditionally
      placeholder={message}
      oninput={(e) =>
        accountSys.setCurCreatingAccount(field, e.currentTarget.value)
      }
    />
  );
};
...
export const SearchTextbox: Component<{ message: string }> = ({ message }) => {
  return (
    <div class={`${TextboxStyle(Size.ui.searchW)} ${searchTextboxStyle}`}>
      ...
      <input class={inputStyle} placeholder={message} oninput={(e) => {mainSys.setSearchQuery(e.currentTarget.value)}} />
    </div>
  );
};
